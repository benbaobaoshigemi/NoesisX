...
- when the figure will enter a formal document, store the final asset in `visualization/` before invoking `push-writer`
...
10. Only after the rendered image has no unresolved defects may you:
   ...
   - hand it to `push-writer` or mention it in a user-facing report
