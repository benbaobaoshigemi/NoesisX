...
5. `brain/GRAPH.md` must use node-ledger form. Do not use append-only event-ledger form as the formal project format.
...
10. `Status History` must preserve the node's lifecycle in order. The node's current status is the last entry.
11. Terminal nodes are `concluded`, `dead`, `absorbed`, `superseded`, and `aborted`. Do not move them back into active states in place.
12. If an old route comes back, create a new node and connect it with `revive_from`. Do not revive a terminal node in place.
13. If a new frame replaces an old frame, create a new node and connect it with `supersede_from`.
14. If several routes collapse into one new judgment, create a new `SYNTHESIS` or `DECISION` node.
15. Treat checkpoints as read-only compression aids, not as a second truth source.
16. Checkpoints must use fixed ids such as `### CP-0001`; do not use free-form checkpoint titles.
17. Before editing an existing `brain/GRAPH.md`, save the exact pre-edit file as a temporary snapshot.
18. After every write to `brain/GRAPH.md`, run:
    - for a new graph file: `python3 .agents/skills/graph-build/scripts/graph_lint.py brain/GRAPH.md`
    - for an existing graph file: `python3 .agents/skills/graph-build/scripts/graph_lint.py brain/GRAPH.md --baseline <pre-edit-snapshot>`
19. If lint fails, fix `brain/GRAPH.md` and rerun. Do not leave the file in a failing state.
20. The baseline check exists to enforce append-only maintenance mechanically: old nodes and old checkpoints must remain unchanged.
21. When changing the linter itself, run:
    - `python3 .agents/skills/graph-build/scripts/test_graph_lint.py`
22. The linter only validates `brain/GRAPH.md` itself plus, when provided, the old `GRAPH.md` snapshot. Cross-file coordination belongs to the parent agent and other skills, not to `GRAPH` lint.
...
- Do not treat checkpoint text as a replacement for node records.
